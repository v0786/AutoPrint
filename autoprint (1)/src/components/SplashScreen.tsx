...
export const SplashScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (isPaused || autoProgress >= 100 || !isShopOnline) return;
  ...
  }, [isPaused, autoProgress, isShopOnline]);

  useEffect(() => {
    if (autoProgress >= 100 && isShopOnline) {
      setStep('specs');
    }
  }, [autoProgress, isShopOnline, setStep]);
  ...
  return (
    <div className="min-h-screen bg-[#0F0F12] text-[#E6E1E9] flex flex-col justify-between items-center p-6 relative overflow-hidden font-sans">
      ...
      {/* Top Bar with Shop Status and Language Switcher */}
      <header className="w-full max-w-md flex items-center justify-between z-10 pt-2 gap-2">
        {isShopOnline ? (
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md text-xs text-zinc-300 shadow-sm">
            <span className="w-2 h-2 rounded-full bg-[#6dd58c] animate-pulse" />
            <span>{t('shopOnline')}</span>
          </div>
        ) : (
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/15 border border-amber-500/30 backdrop-blur-md text-xs text-amber-300 shadow-sm">
            <AlertCircle className="w-3.5 h-3.5 text-amber-400" />
            <span>{t('noShopSelected')}</span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 backdrop-blur-md text-xs text-[#D0BCFF] hover:border-[#D0BCFF]/30 transition-all cursor-pointer shadow-sm"
          >
      ...
      <div className="w-full max-w-md my-auto flex flex-col items-center text-center z-10 py-6">
          ...
          {isShopOnline && currentShop ? (
            <div>
              <div className="flex items-start justify-between gap-3 mb-3">
                ...
                <button
                  ...
                  className="text-[11px] font-semibold text-[#D0BCFF] hover:underline flex-shrink-0 mt-0.5 cursor-pointer"
                >
                  {t('shopDetails')}
                </button>
              </div>
              ...
              <div className="bg-black/30 rounded-2xl p-3 text-xs text-zinc-400 space-y-2 border border-white/5">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-mono text-[#D0BCFF] font-semibold">
                    {currentShop.kioskNumber.replace(/Kiosk/gi, 'Shop')}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span>{t('avgPrintSpeed')}</span>
            ...
            <div className="text-center py-2 space-y-2">
              ...
              <button
                ...
                className="mt-2 px-4 py-2 rounded-xl bg-[#D0BCFF] text-[#381E72] text-xs font-black hover:bg-[#decbf7] transition-all cursor-pointer shadow-md"
              >
                {t('connectVerifiedShop')}
              </button>
            </div>
          )}
        </motion.div>
        ...
        >
          ...
          >
            <span>{t('startPrintJob')}</span>
            <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
          </button>

          {/* Auto-transition bar if shop is online */}
          {isShopOnline && (
            <>
              <div
              ...
              >
                <span>{isPaused ? t('autoStartPaused') : t('autoEntering')}</span>
