...
import { JuspayModal } from './JuspayModal';
...
export const PaymentStep: React.FC = () => {
  ...
  const [selectedMethod, setSelectedMethod] = useState<PaymentMethod>(paymentDetails.method || 'razorpay');
  const [selectedUpiApp, setSelectedUpiApp] = useState<UpiAppId>(paymentDetails.upiApp || 'gpay');
  const [copiedUpi, setCopiedUpi] = useState(false);
  const [isProcessing, setIsProcessing] = useState(false);
  const [showConfirmModal, setShowConfirmModal] = useState(false);
  const [upiTimerSeconds, setUpiTimerSeconds] = useState(180); // 3-minute timer
  ...
  const handleProceed = () => {
    ...
    if (selectedMethod === 'juspay') {
      initiatePayment('juspay', { gateway: 'juspay' });
  ...
  const handleGatewaySuccess = (gateway: GatewayType, paymentId: string) => {
    setIsRazorpayOpen(false);
    setIsJuspayOpen(false);
    completePayment({
      method: gateway === 'razorpay' ? 'razorpay' : 'juspay',
      gateway,
  ...
  const shopName = isShopOnline && currentShop ? currentShop.name : 'No shop selected';
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pb-32 sm:px-6 pt-2 space-y-6 font-sans">
      ...
      <div className="bg-white/5 border border-white/10 backdrop-blur-xl rounded-3xl p-6 sm:p-8 shadow-xl shadow-black/40 space-y-5">
        <div>
          <h2 className="text-lg font-bold text-white">Choose Payment Gateway / Method</h2>
          <p className="text-xs text-zinc-400">
            Pay online via Razorpay or Juspay, direct shop UPI QR, or cash at counter
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-zinc-400">UPI, Credit/Debit Cards, NetBanking</div>
                </div>
              </div>
            ...
            <p className="text-xs text-zinc-400 mt-2 leading-relaxed">
              Official Razorpay checkout with all major Indian banks, cards, wallets & UPI apps.
            </p>
          </div>

          {/* Option 2: Juspay (1-Click Express Checkout) */}
          <div
            onClick={() => setSelectedMethod('juspay')}
            className={`p-5 rounded-3xl border-2 transition-all cursor-pointer relative flex flex-col justify-between ${
              selectedMethod === 'juspay'
                ? 'bg-[#063b28]/40 border-emerald-400 text-white shadow-xl shadow-emerald-500/10'
                : 'bg-black/30 border-white/10 hover:border-white/20 text-zinc-300'
            }`}
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${
                  selectedMethod === 'juspay' ? 'bg-emerald-500 text-white' : 'bg-white/5 text-emerald-400'
                }`}>
                  <Zap className="w-5 h-5 fill-current" />
                </div>
                <div>
                  <div className="text-sm font-bold flex items-center gap-1.5">
                    <span>Juspay HyperCheckout</span>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-400/30">
                      1-CLICK
                    </span>
                  </div>
                  <div className="text-xs text-zinc-400">Direct Express UPI & Smart Router</div>
                </div>
              </div>
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                selectedMethod === 'juspay' ? 'border-emerald-400 bg-emerald-500' : 'border-white/20'
              }`}>
                {selectedMethod === 'juspay' && <div className="w-2 h-2 rounded-full bg-white" />}
              </div>
            </div>

            <p className="text-xs text-zinc-400 mt-2 leading-relaxed">
              Fastest 1-click UPI redirection with automated failure fallback.
            </p>
          </div>

          {/* Option 3: Shop Direct UPI QR */}
          <div
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-bold">Shop UPI QR Code</div>
                  <div className="text-xs text-zinc-400">Scan live QR directly on screen</div>
                </div>
              </div>
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
            ...
            <p className="text-xs text-zinc-400 mt-2 leading-relaxed">
              Scan dynamic shop VPA code via GPay, PhonePe, Paytm or BHIM on your phone.
            </p>
          </div>

          {/* Option 4: Cash at Counter */}
          <div
          ...
          >
            ...
            <p className="text-xs text-zinc-400 mt-2 leading-relaxed">
              Your collection code will show <strong className="text-[#ffb77c]">"Cash Due: ₹{pricing.totalAmount.toFixed(2)}"</strong>.
            </p>
          </div>

        </div>

        {/* DYNAMIC UPI ACCORDION / QR BOX IF 'upi' METHOD SELECTED */}
        {selectedMethod === 'upi' && (
          <motion.div
          ...
          >
            ...
            {/* Dynamic Scannable UPI QR Frame */}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 bg-[#0F0F12]/80 backdrop-blur-xl border-t border-white/10 p-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
          <div>
            <span className="text-[11px] text-zinc-400 block capitalize">
              Method:{' '}
              {selectedMethod === 'razorpay'
                ? 'Razorpay Gateway'
                : selectedMethod === 'juspay'
      ...
      {/* JUSPAY CHECKOUT MODAL */}
      <JuspayModal
        isOpen={isJuspayOpen}
        onClose={() => setIsJuspayOpen(false)}
        onSuccess={(paymentId) => handleGatewaySuccess('juspay', paymentId)}
      />
