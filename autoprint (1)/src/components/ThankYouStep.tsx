...
export const ThankYouStep: React.FC = () => {
  ...
  const handleShare = () => {
    const text = `AutoPrint Collection Code: ${currentOrder.collectionCode} at ${shopName} (${shopNumber}). Amount: ₹${currentOrder.pricing.totalAmount}.`;
    if (navigator.share) {
      navigator.share({
  ...
  const isRazorpay = currentOrder.payment.method === 'razorpay' || currentOrder.payment.gateway === 'razorpay';
  const isJuspay = currentOrder.payment.method === 'juspay' || currentOrder.payment.gateway === 'juspay';
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pb-24 sm:px-6 pt-2 space-y-6 font-sans">
      ...
      >
        ...
        {autoDownloaded && (
          ...
          >
            ...
            <span>Digital receipt saved locally & automatically</span>
          </motion.div>
        )}
      </motion.div>
      ...
      >
        ...
        {/* Razorpay / Juspay Gateway Badge if Online */}
        {isRazorpay && (
          <div className="mt-5 p-3.5 rounded-2xl bg-blue-500/15 border border-blue-500/30 text-blue-200 text-xs flex items-center justify-center gap-2 font-medium">
            <ShieldCheck className="w-4 h-4 text-blue-400" />
            <span>
              Paid via Razorpay: <span className="font-mono text-white font-bold">{currentOrder.payment.gatewayPaymentId || 'Authorized'}</span>
            </span>
          </div>
        )}

        {isJuspay && (
          <div className="mt-5 p-3.5 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-200 text-xs flex items-center justify-center gap-2 font-medium">
            <Zap className="w-4 h-4 text-emerald-400 fill-current" />
            <span>
              Paid via Juspay HyperCheckout: <span className="font-mono text-white font-bold">{currentOrder.payment.gatewayPaymentId || 'Authorized'}</span>
            </span>
          </div>
        )}
        ...
        {isHeavyWorkload && (
          <div className="mt-4 p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs flex items-center justify-center gap-2 font-medium">
            <AlertTriangle className="w-4 h-4 text-amber-400 flex-shrink-0" />
            <span>
              Heavy shop workload: Your prints will be held for 2 hours. Use your collection code at the counter after payment.
            </span>
          </div>
        )}
      </motion.div>
      ...
      >
        ...
        <div className="bg-black/30 rounded-2xl p-3.5 text-xs text-zinc-400 flex items-center justify-between border border-white/5">
          <div className="flex items-center gap-2">
            ...
            <span>Shop Hardware: <strong className="text-white">{currentShop?.activePrinters?.[0] || 'Enterprise Production Laser'}</strong></span>
          </div>
          <span className="text-[#D0BCFF] font-mono font-semibold">{shopNumber}</span>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="space-y-2.5 text-xs text-zinc-400">
          <div className="flex justify-between">
            ...
            <span className="text-white font-semibold">{shopName} {currentShop?.branch ? `(${currentShop.branch})` : ''}</span>
          </div>
          <div className="flex justify-between">
            <span>Document:</span>
          ...
          <div className="flex justify-between">
            ...
            <span className="text-[#6dd58c] font-semibold uppercase">
              {currentOrder.payment.method === 'razorpay'
                ? 'Razorpay Gateway'
                : currentOrder.payment.method === 'juspay'
                ? 'Juspay HyperCheckout'
                : currentOrder.payment.method === 'upi'
                ? `UPI (${currentOrder.payment.upiApp?.toUpperCase()})`
                : 'Pay Cash at Counter'}
            </span>
          </div>
          {currentOrder.payment.gatewayPaymentId && (
            <div className="flex justify-between">
              <span>Transaction Ref:</span>
              <span className="font-mono text-zinc-200">{currentOrder.payment.gatewayPaymentId}</span>
            </div>
          )}
