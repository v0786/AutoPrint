/**
 * Customer Web Utility Functions
 *
 * Purpose:
 *   Helper functions for the customer kiosk frontend:
 *   - Collection code generation (for display purposes)
 *   - Order ID generation
 *   - File size formatting
 *   - Page range parsing
 *
 * Note:
 *   The ACTUAL verification code is generated server-side by the backend.
 *   generateCollectionCode() here is for display/UI only (may not be used).
 *   Real verification codes come from POST /api/jobs response.
 */
 ...
 * The actual verification code is generated server-side and returned in API response.
 *
 * Inputs: None
 * Returns: String in format "XXXX-XXXX"
 */
...
export function downloadReceipt(order: PrintOrder, shop: ShopInfo): void {
  ...
  const lines = [
    ...
    `Subtotal        : Rs. ${order.pricing.subtotal.toFixed(2)}`,
    `GST & Handling  : Rs. ${order.pricing.gstAmount.toFixed(2)}`,
