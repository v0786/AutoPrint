...
export const QrCodeModal: React.FC = () => {
  ...
  const currentUrl = typeof window !== 'undefined'
    ? `${window.location.origin}${window.location.pathname}?shop=${currentShop.id}`
    : `https://autoprint.vercel.app/?shop=${currentShop.id}`;
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md font-sans">
        <motion.div
          ...
          className="w-full max-w-md bg-[#141419]/95 rounded-3xl p-6 sm:p-7 border border-white/10 backdrop-blur-2xl shadow-2xl space-y-5 text-center"
        >
          {/* Header */}
          ...
          <div className="p-6 bg-gradient-to-b from-[#ffffff] to-[#ede8f5] rounded-3xl text-gray-900 shadow-xl border-4 border-white/20 max-w-[280px] mx-auto relative">
            ...
            <div className="text-[9px] text-gray-500 font-mono mt-0.5">
              Counter: {currentShop.kioskNumber}
            </div>
          </div>
