 ...
 * Simulates UPI Intent, Dynamic QR generation, failure diagnostics,
 * and records attempts on the backend REST API.
 */
...
class UpiPaymentGatewayService {
   ...
   */
  public async initializeUpiIntent(
    verificationCode: string,
    amount: number,
    currency = 'INR'
  ): Promise<UpiIntentSession> {
    const record = (await verificationService.lookupByCode(verificationCode)) || verificationService.getRecord(verificationCode);
    if (!record) {
  ...
  /**
   * Simulates a successful UPI authorization response and records it on backend.
   */
  public async simulateSuccessfulUpiPayment(
    ...
    const { record: updatedRecord, strikeLockoutTriggered } =
      await verificationService.recordDigitalPaymentAttempt(verificationCode, {
        method: 'UPI',
  ...
  /**
   * Simulates a failed UPI authorization response to test fail-safe retry limits and 3-strike lockout.
   */
