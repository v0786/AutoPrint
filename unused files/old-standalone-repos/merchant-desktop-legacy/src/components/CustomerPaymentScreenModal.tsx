...
export const CustomerPaymentScreenModal: React.FC<CustomerPaymentScreenModalProps> = ({
  ...
  const handleSimulateUpiSuccess = async () => {
    ...
    try {
      const updated = await paymentGatewayService.simulateUpiPaymentSuccess(
  ...
  const handleSimulateUpiFailure = async () => {
    ...
    setErrorMessage(null);
    try {
      const result = await paymentGatewayService.simulateUpiPaymentFailure(record.verificationCode);
      if (onPaymentUpdated) onPaymentUpdated(result.record);
    } catch (err: any) {
