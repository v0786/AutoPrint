...
export const StaffVerificationView: React.FC<StaffVerificationViewProps> = ({
  ...
  const performLookup = (code: string) => {
    const found = verificationService.lookupByCode(code);
    if (found) {
  ...
  const handleImmediateUpiHandover = async () => {
    ...
    try {
      const updated = verificationService.confirmDocumentHandover(
        activeRecord.verificationCode,
        'STAFF-DESK-01',
  ...
  const handleConfirmCashCollection = async () => {
    ...
    try {
      ...
      const paidRecord = verificationService.recordCashCollection(
        activeRecord.verificationCode,
        tendered,
        'STAFF-DESK-01',
        'Sarah K. (Station Staff)'
      ...
      const finalRecord = verificationService.confirmDocumentHandover(
