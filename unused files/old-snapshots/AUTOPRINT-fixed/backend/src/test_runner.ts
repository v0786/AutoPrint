...
import { VerificationAuditLog } from './types';
...
const newJob = AutoPrintService.submitJob({
  ...
  amountTotal: 45.00,
  paymentMethod: 'UPI',
  rawContentHtml: '<div><h1>Annual Financial Report</h1><p>Confidential pages 1-10</p></div>',
  specs: {
    copies: 2,
    colorMode: 'color',
    paperSize: 'a4',
...
const cashRecord = VerificationService.processCashCollection(code, 50.00, 'STAFF-DESK-99', 'Bob Cashier');
console.log(`✓ Cash Collected! Tendered: $${cashRecord.cashTenderedAmount}, Total Due: $${cashRecord.amountTotal}, Change Due: $${cashRecord.cashChangeDue}`);
...
logs.forEach((log: VerificationAuditLog, index: number) => {
