...
export const Header: React.FC<HeaderProps> = ({
  ...
  const customerAccessUrl = typeof window !== 'undefined'
    ...
    : `http://localhost:8085/customer?station=${stationId}`;
        ...
        <div className="relative ml-1">
          ...
          {isProfileOpen && (
            ...
            >
              ...
              {/* Configure Payment (QR & UPI) Button */}
              <button
                onClick={() => {
                  setIsProfileOpen(false);
                  window.location.hash = '#payment-config';
                }}
                className="w-full bg-slate-100 hover:bg-blue-50 text-slate-700 hover:text-blue-800 border border-slate-200 hover:border-blue-300 py-2 px-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all"
              >
                <QrCode className="w-3.5 h-3.5 text-blue-600" />
                <span>Configure QR Code & UPI ID</span>
              </button>
            </div>
          )}
        </div>
