...
$listeningPorts = & netstat -ano
$port4100 = $listeningPorts | Select-String ":4100"
if ($port4100) {
...
$port5173 = $listeningPorts | Select-String ":5173"
if ($port5173) {
...
Write-Host ""
Write-Host "[CHECK 9] npm cache availability..." -ForegroundColor Cyan
$npmCachePath = & npm config get cache
if ($LASTEXITCODE -eq 0 -and $npmCachePath -and (Test-Path $npmCachePath)) {
    Write-Host "  [PASS] npm cache directory found" -ForegroundColor Green
} else {
    Write-Host "  [INFO] npm cache directory is not available yet" -ForegroundColor Cyan
}
